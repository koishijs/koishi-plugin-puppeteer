...
class Puppeteer extends Service {
  ...
  constructor(
    ctx: Context,
    public config: Puppeteer.Config,
  ) {
    super(ctx, "puppeteer");
    ctx.plugin(Canvas);
  }
  ...
  async start() {
    ...
    if (!executablePath) {
      try {
        ...
        this.ctx.logger.info(
          `${product || "chrome"} executable found at %c`,
          executablePath,
        );
      } catch (err) {
        this.ctx.logger.warn(
          `failed to find ${product || "chrome"} executable:`,
          err,
        );
      }
    }
    ...
    const transformStyle = (source: {}, base = {}) => {
      return Object.entries({ ...base, ...source })
        .map(([key, value]) => {
    ...
    this.ctx.component("html", async (attrs, children, _session) => {
      ...
      const transform = (element: h) => {
        ...
        return h(
          element.type,
          attrs,
          element.children.map(transform).filter(Boolean),
        );
      };
      ...
      try {
        if (attrs.src) {
          ...
          await page.goto(
            pathToFileURL(resolve(__dirname, "../index.html")).href,
          );
          const bodyStyle =
            typeof attrs.style === "object"
              ? transformStyle({ display: "inline-block" }, attrs.style)
              : ["display: inline-block", attrs.style]
                  .filter(Boolean)
                  .join("; ");
...
namespace Puppeteer {
  ...
  export const Config = Schema.intersect([
    Schema.object({
      ...
      product: Schema.union(["chrome", "firefox"])
        .description("浏览器类型，支持 Chrome 和 Firefox。")
        .default("chrome"),
